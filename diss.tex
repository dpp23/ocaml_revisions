...
There exists a successful implementation of Concurrent revisions in Haskell, which shows that this concept has a place in functional environment as well[8].
...
Here {\tt x} is declared as a {\tt int ref} and is a reference to a particular cell that contains an integer value. The value of {\tt x} itself cannot be changed. What can be changes of the value inside the cell. On line 3 {\tt x} is updated by assigning to it the sum of the dereferenced value of {\tt x} and 2.  
...
Revision diagrams are an intuitive graphical representation of the revision flow. In Fig.1.1 is an example of a revision diagram for Example 1. 
