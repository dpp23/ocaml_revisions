...
let merger a _ c = match c.last_event with
                    |Register (name) -> begin match List.hd c.users with
                                         ...
                                         |Some(user) ->
                                          ...
                                          List.iter a.rooms (fun room -> 
                                                               ...
                                                               Writer.write_sexp user.writer (sexp_of_command (Room(room.id))));
                    ...
                    |Enter (room_id, id) -> begin match (find_room_by_id a.rooms room_id, find_room_by_id c.rooms room_id) with
                                             ...
                                             |(None,_) -> !state
                                             |(Some(a_room), Some(c_room)) -> match (find_user_by_id a_room.users id, find_user_by_id c_room.users id) with
                                                              |(None, Some(user)) ->
                                                                 send_to_user_list (user::a_room.users) (Enter(room_id, id)); 
                    ...
                    |Merge (_,_) -> raise (Horror("Not implemented")) (*TODO:implement*)
...
let handle rev action w r = match action with
                            | Register (name) -> 
                                  SvRev.read rev iso 
...
let rec process_queue rev = match Queue.dequeue queue with
                              |Some(action, r, w) -> 
                                  ignore(SvRev.fork rev (fun rev -> handle rev action w r)
                                  >>|(fun rev2 -> process_queue (SvRev.join rev rev2))); () 
                                                  
                              |None -> ignore(return (process_queue rev)); () 
...
let rec serve r w = Reader.read_sexp r 
                         >>= (function
                             | `Eof -> return ()
                             | `Ok s -> let event = command_of_sexp s and st = !state in
                                          match event with
                                         | Register (name) -> 
                                               if List.exists st.users (fun user -> user.name = name) then 
                                                                         Writer.write_sexp w 
                                                                                   (sexp_of_command 
                                         ...
                                         | Create (room_id, _) ->
                                              if exists_room_by_id st.rooms room_id then begin
                                                                         ...
                                                                         else begin Queue.enqueue queue (event, r, w); serve r w end
                                         | Enter (room_id, id) -> 
                                                                           ...
                                                                           |(Some(a), Some(u),Some(r_)) -> 
                                                                                 match find_user_by_id r_.users a.id with
                                                                                   ...
                                                                                   |Some(admin) -> if not admin.su then begin
                                                                                                    ...
                                                                                                    |Some(_) ->
                                                                                                          Queue.enqueue queue (event, r, w); serve r w 
...
let () =
  Command.async_basic
    ...
    (fun port () -> ignore((return staterev) >>| process_queue ); run ~port)
  |> Command.run
