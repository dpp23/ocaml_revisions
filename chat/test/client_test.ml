...
let print_message (m:message) =  
 let pretty_print name = 
   let k = sexp_of_mes(Receive (int_of_string(m.text), m.timestamp)) in
     print_string ((Sexp.to_string_mach k) ^ "\n") in                                                     
  match find_user_by_id !users m.user_id with
  |None -> pretty_print "Unknown"
  |Some(u) -> pretty_print u.name

let print_message_send (m:message) =  
 let pretty_print name = 
   let k = sexp_of_mes(Send (int_of_string(m.text), m.timestamp)) in
     print_string ((Sexp.to_string_mach k) ^ "\n") in                                                     
  match find_user_by_id !users m.user_id with
  |None -> pretty_print "Unknown"
  |Some(u) -> pretty_print u.name
...
let parse_command line = try begin
  if Str.string_match regexp_enter line 0 then
    ...
    match split with
      Some (room, message) -> let m = {timestamp = Time.now();
                                  user_id = !id;
                                  text = message;
                                  room_id = (int_of_string room)
                                 } in print_message_send m; (Message m)
